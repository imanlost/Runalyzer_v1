...
const Icons = {
    ...
    CloudDownload: () => <i className="fa-solid fa-cloud-arrow-down text-blue-400"></i>,
    Settings: () => <i className="fa-solid fa-gear text-gray-400"></i>,
...
const IntervalsConfigModal = ({ isOpen, onClose, config, onSave, isSyncing, onSync }: any) => {
    const [athleteId, setAthleteId] = useState(config.athleteId || '');
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#1C1C1E] border border-white/10 rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-[#34C759]">Sincronizar Intervals.icu</h2>
                    <button onClick={onClose} className="p-2 text-gray-400 hover:text-white rounded-lg hover:bg-white/10 transition-colors"><Icons.XMark /></button>
                </div>
                
                <div className="space-y-4">
                    <div>
                        <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Athlete ID</label>
                        <input type="text" value={athleteId} onChange={e => setAthleteId(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-sm text-white focus:border-blue-500 outline-none" placeholder="Ej: i123456" />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-gray-400 uppercase mb-2">API Key</label>
                        <input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-sm text-white focus:border-blue-500 outline-none" placeholder="Tu API Key" />
                        <p className="text-[10px] text-gray-500 mt-2">Encuentra tu Athlete ID y API Key en Settings dentro de intervals.icu</p>
                    </div>

                    <div className="flex space-x-3 pt-4">
                        <button onClick={() => onSave({ athleteId, apiKey })} className="flex-1 bg-white/10 hover:bg-white/20 text-white py-3 rounded-xl text-sm font-bold transition-colors">
                            Guardar
                        </button>
                        <button onClick={onSync} disabled={isSyncing || !athleteId || !apiKey} className={`flex-1 flex items-center justify-center py-3 rounded-xl text-sm font-bold transition-all ${isSyncing || !athleteId || !apiKey ? 'bg-blue-500/50 cursor-not-allowed opacity-50' : 'bg-gradient-to-r from-blue-600 to-[#34C759] hover:opacity-90 shadow-[0_0_15px_rgba(52,199,89,0.3)]'}`}>
                            {isSyncing ? 'Cargando...' : <><span className="mr-2"><Icons.CloudDownload /></span> Iniciar Sync</>}
                        </button>
                    </div>
                </div>
            </div>
    ...
    return (
        <aside className="w-80 bg-[#1C1C1E]/95 backdrop-blur border-r border-white/10 flex flex-col z-20 transition-all duration-300">
            <div className="p-4 border-b border-white/10">
                ...
                <div 
                    className="border-2 border-dashed border-white/10 rounded-xl p-4 text-center hover:border-[#34C759] hover:bg-white/5 transition-all cursor-pointer mb-2 group"
                ...
                <div 
                    className="w-full bg-[#34C759]/10 border border-[#34C759]/30 text-[#34C759] rounded-xl p-3 text-center cursor-pointer mb-4 font-bold text-xs flex items-center justify-center transition-all hover:bg-[#34C759]/20"
                    onClick={openIntervalsConfig}
                >
                    <span className={`mr-2 ${isSyncing ? 'animate-bounce' : ''}`}><Icons.CloudDownload /></span> {isSyncing ? 'Sincronizando...' : 'Conectar Intervals.icu'}
                </div>
...
const App = () => {
    ...
    useEffect(() => {
        ...
        const savedIntervals = localStorage.getItem('runalyzer_intervals');
        if (savedIntervals) setIntervalsConfig(JSON.parse(savedIntervals));
    ...
    const syncIntervals = async () => {
        ...
        try {
            ...
            const existingIds = new Set(sessions.map(s => s.id));
            const activitiesToFetch = activities.filter((a: any) => !existingIds.has(`intervals-${a.id}`)).slice(0, 50);

            if (activitiesToFetch.length === 0) {
                addNotification({ type: 'info', message: 'No hay actividades nuevas en Intervals.icu.' });
                setIsSyncing(false);
                return;
            }

            addNotification({ type: 'info', message: `Descargando GPS de ${activitiesToFetch.length} actividades...` });
            let loaded: Session[] = [];

            for (const act of activitiesToFetch) {
                try {
                    const streamsRes = await fetch(`https://intervals.icu/api/v1/activity/${act.id}/streams`, { headers: { 'Authorization': authHeader } });
                    if (!streamsRes.ok) continue;
                    
                    const streamsJson = await streamsRes.json();
                    if (!Array.isArray(streamsJson)) continue;

                    const streamMap: Record<string, any[]> = {};
                    streamsJson.forEach((s: any) => { streamMap[s.type] = s.data; });
                    const trackPoints: TrackPoint[] = [];
                    const len = streamMap['time']?.length || 0;
                    const startTimeEpoch = new Date(act.start_date_local).getTime();

                    for (let i = 0; i < len; i++) {
                        const latlng = streamMap['latlng']?.[i];
                        trackPoints.push({
                            timestamp: new Date(startTimeEpoch + (streamMap['time'][i] * 1000)).toISOString(),
                            lat: latlng ? latlng[0] : 0,
                            lon: latlng ? latlng[1] : 0,
                            dist: streamMap['distance']?.[i] || 0,
                            altitude: streamMap['altitude']?.[i] || 0,
                            hr: streamMap['heartrate']?.[i] || 0,
                            speed: streamMap['velocity_smooth']?.[i] || 0,
                            cadence: streamMap['cadence']?.[i] || 0
                        });
                    }

                    if (trackPoints.length < 2) continue;

                    let sport = act.type.toUpperCase();
                    if (sport === 'RIDE' || sport === 'VIRTUALRIDE') sport = 'CYCLING';

                    let maxHr = act.max_heartrate || 0;
                    let distance = act.distance || 0;
                    let duration = act.elapsed_time || 0;

                    let acsmVo2Max = calculateACSMVo2(trackPoints, maxHr || 190, 60);
                    const vam6min = calculateSlidingWindowMaxSpeed(trackPoints, 360);
                    if (acsmVo2Max === 0 && vam6min > 0) acsmVo2Max = 3.5 * (vam6min * 3.6);

                    // Parche de elevación si no existe o es 0
                    let elevation = act.total_elevation_gain || 0;
                    if (elevation === 0) {
                         let gain = 0;
                         for(let i=1; i<trackPoints.length; i++) {
                    ...
                    loaded.push({
                        id: `intervals-${act.id}`,
                        name: act.name || generateSessionName(sport, act.start_date_local, distance),
                        startTime: act.start_date_local,
                        duration, distance, sport, avgHr: act.average_heartrate || 0, maxHr,
                        calories: act.calories || 0, totalElevationGain: elevation,
                        avgCadence: act.average_cadence || 0, vam6min,
                        best20minSpeed: calculateSlidingWindowMaxSpeed(trackPoints, 1200),
                        acsmVo2Max, trackPoints, trimp: calculateTRIMP(trackPoints, maxHr || 190, 60),
                        climbScore: calculateClimbScore(elevation, distance)
                    });
                } catch (e) { console.error("Error process act", act.id, e); }
            }

            if (loaded.length > 0) {
               handleDataLoaded(loaded);
               setShowIntervalsConfig(false);
            } else {
               addNotification({ type: 'info', message: 'No se encontraron datos GPS en las actividades recientes.' });
            }
        } catch (error: any) {
             addNotification({ type: 'error', message: `Error credenciales o API: ${error.message}` });
             setShowIntervalsConfig(true);
        } finally {
            setIsSyncing(false);
        }
    ...
    return (
        <div className="flex h-screen bg-black text-white font-sans overflow-hidden selection:bg-[#34C759] selection:text-black">
            ...
            <IntervalsConfigModal
                isOpen={showIntervalsConfig}
                onClose={() => setShowIntervalsConfig(false)}
                config={intervalsConfig}
                onSave={(cfg: any) => {
                    setIntervalsConfig(cfg);
                    localStorage.setItem('runalyzer_intervals', JSON.stringify(cfg));
                    addNotification({ type: 'success', message: 'Credenciales guardadas. Puedes iniciar la sincronización.' });
                    setShowIntervalsConfig(false);
                }}
                isSyncing={isSyncing}
                onSync={syncIntervals}
            />
        </div>
    );
};
